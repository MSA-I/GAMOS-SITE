...
const PAIRS = [
  ...
  ["גן אירועים, אולם וריזורט עם 33 סוויטות — האורחים נשארים ללון, והאירוע הופך לחוויה של יממה מול נוף מדבר יהודה.", "An events garden, hall and resort with 33 suites — guests stay overnight, turning the celebration into a full-day experience facing the Judean Desert.", "Un jardin événementiel, une salle et un resort avec 33 suites — les invités restent dormir sur place et la fête devient une expérience d'une journée entière face au désert de Judée."],
  ...
  // Buffet cards (2026-07-13: folded into the culinary grid as 6 "בופה" cards)
  ["בופה", "Buffet", "Buffet"],
  ["פיצה נאפוליטנית", "Neapolitan Pizza", "Pizza napolitaine"],
  ["נאפית בתנור אבן לוהט מול האורחים.", "Baked in a blazing stone oven before your guests.", "Cuite dans un four en pierre ardent devant vos convives."],
  ["מגש סושי קשת", "Rainbow Sushi Platter", "Plateau de sushis arc-en-ciel"],
  ["סושי-בר עשיר בעבודת יד.", "A rich, handmade sushi bar.", "Un bar à sushis riche, fait main."],
  ["פוקאצ'ה ופסטו בזיליקום", "Focaccia & Basil Pesto", "Focaccia et pesto au basilic"],
  ["חמה מהטאבון, בהזלפת שמן עשבים.", "Warm from the taboon, drizzled with herb oil.", "Chaude du four taboon, arrosée d'huile aux herbes."],
  ["שווארמת פרגית על האש", "Grilled Chicken Shawarma", "Shawarma de poulet grillé"],
  ["שיפוד מסתובב מעל גחלים לוהטות.", "A skewer turning over glowing coals.", "Une broche tournant au-dessus de braises ardentes."],
  ["טרטר בהרכבה חיה", "Tartare Assembled to Order", "Tartare assemblé à la minute"],
  ["עמדת שף חיה מול האורחים.", "A live chef station before your guests.", "Une station de chef en direct devant vos convives."],
  ["מאפי בוטיק מהטאבון", "Boutique Taboon Pastries", "Pâtisseries de boutique au four taboon"],
  ["מאפים מוזהבים על מעמדי עץ, ישר מהטאבון.", "Golden pastries on wooden stands, straight from the taboon.", "Des pâtisseries dorées sur des présentoirs en bois, tout droit du four taboon."],
  ...
  // #events fixed-preview gallery (2026-07-13 v3: name + the ORIGINAL full
  // description per event type, restored per user request). "סוגי אירועים"
  // doubles as the nav label since the nav swap — keep the translation short.
  ["סוגי אירועים", "Celebrations", "Célébrations"],
  ...
  ["כשרות בפיקוח הרבנות · אפשרות להתאמת הרמה לצורכי האירוע", "Kashrut under Rabbinate supervision · The level can be adapted to your event", "Cacherout sous la supervision du Rabbinat · Le niveau peut être adapté à votre événement"],
  ...
  ["שדות חובה: שם, טלפון, דוא\"ל וסוג אירוע.", "Required fields: name, phone, email and event type.", "Champs obligatoires : nom, téléphone, e-mail et type d'événement."],
  ["נחזור אליכם בתוך יום עסקים.", "We'll get back to you within one business day.", "Nous vous répondrons sous un jour ouvré."],
  ...
  // Runtime contact feedback. These strings also live in a hidden DOM copy bank
  // so js/contact-form.js can read the currently translated text after a switch.
  ["אנא הזינו שם מלא.", "Please enter your full name.", "Veuillez indiquer votre nom complet."],
  ["אנא הזינו מספר טלפון.", "Please enter a phone number.", "Veuillez indiquer un numéro de téléphone."],
  ["מספר הטלפון אינו בפורמט המתאים. נסו 050-1234567 או 077-9972343.", "This phone number isn't in the expected format. Try 050-1234567 or 077-9972343.", "Le numéro de téléphone n'est pas au bon format. Essayez le 050-1234567 ou le 077-9972343."],
  ["אנא הזינו כתובת דוא\"ל.", "Please enter your email address.", "Veuillez indiquer votre adresse e-mail."],
  ["כתובת הדוא\"ל צריכה לכלול @ ודומיין. לדוגמה: name@example.com.", "Email addresses need an @ symbol and a domain. Example: name@example.com.", "L'adresse e-mail doit contenir un @ et un nom de domaine. Exemple : name@example.com."],
  ["אנא בחרו סוג אירוע.", "Please choose an event type.", "Veuillez choisir un type d'événement."],
  ["בדקו את השדות המסומנים ונסו שוב.", "Check the highlighted fields and try again.", "Vérifiez les champs indiqués et réessayez."],
  ["לא הצלחנו להכין טיוטת WhatsApp. אפשר לשלוח את הפרטים בדוא\"ל.", "We couldn't prepare the WhatsApp draft. You can send your details by email.", "Nous n'avons pas pu préparer le brouillon WhatsApp. Vous pouvez envoyer vos coordonnées par e-mail."],
  ["לא הצלחנו לפתוח ערוץ שליחה. נסו שוב או התקשרו אלינו.", "We couldn't open a sending option. Try again or call us.", "Nous n'avons pas pu ouvrir de canal d'envoi. Réessayez ou appelez-nous."],
  ["פתחנו טיוטת הודעה ב-WhatsApp. בדקו את הפרטים ולחצו שם על שליחה.", "We opened a message draft in WhatsApp. Check the details, then press Send in WhatsApp.", "Nous avons ouvert un brouillon dans WhatsApp. Vérifiez les informations, puis appuyez sur Envoyer dans WhatsApp."],
  ["לא הצלחנו לפתוח את WhatsApp אוטומטית. לחצו על הקישור כדי לפתוח את טיוטת ההודעה.", "We couldn't open WhatsApp automatically. Use the link to open the message draft.", "Nous n'avons pas pu ouvrir WhatsApp automatiquement. Utilisez le lien pour ouvrir le brouillon."],
  ["פתחו טיוטה ב-WhatsApp", "Open the draft in WhatsApp", "Ouvrir le brouillon dans WhatsApp"],
  ["שלחו את הפרטים בדוא\"ל", "Send the details by email", "Envoyer les coordonnées par e-mail"],
  ["שלום, הגעתי דרך האתר.", "Hello, I came through the website.", "Bonjour, je viens du site web."],
  ["פנייה מהאתר — גאמוס", "Website inquiry — Gamos", "Demande depuis le site — GAMOS"],
  ...
  // Site footer (2026-07-21, footer-craft pass). Reuses existing keys for the
  // CTA pair, phone/whatsapp/email/address labels and "די זהב 7…".
  ["האירוע והחופשה — במקום אחד", "The celebration and the getaway — in one place", "La fête et le séjour — en un seul lieu"],
  ["נשמח לארח את האירוע שלכם.", "We'd love to host your celebration.", "Nous serions ravis d'accueillir votre événement."],
  ["בואו נבדוק יחד תאריך פנוי, נסייר במתחם, ונתכנן חוויה שנשארת הרבה אחרי שהערב נגמר.", "Let's check an open date together, tour the estate, and plan an experience that lingers long after the evening ends.", "Vérifions ensemble une date disponible, visitons le domaine et imaginons une expérience qui perdure bien après la fin de la soirée."],
  ["ליצירת קשר", "Get in Touch", "Nous contacter"],
  ["עקבו אחרינו", "Follow Us", "Suivez-nous"],
  ["אינסטגרם", "Instagram", "Instagram"],
  ["פייסבוק", "Facebook", "Facebook"],
  ["מדיניות פרטיות", "Privacy Policy", "Politique de confidentialité"],
  ["תנאי שימוש", "Terms of Use", "Conditions d'utilisation"],
  ["הצהרת נגישות", "Accessibility Statement", "Déclaration d'accessibilité"],
  ["קישורים משפטיים", "Legal links", "Liens juridiques"],
  ["חזרה לראש העמוד", "Back to top", "Retour en haut"],
  ["גאמוס · כל הזכויות שמורות", "Gamos · All rights reserved", "GAMOS · Tous droits réservés"],
];
