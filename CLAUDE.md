...
2. **שני תיקונים חובה לפני העלייה לאוויר:**
   - **(א)** ✅ **בוצע 2026-06-16.** ה-scrub הקולינרי הומר מ-361 פריימי WebP (4K, 209MB)
     ל-**`culinary-h264-1080.mp4`** — סרטון יחיד שנגלל ב-`scroll-scene.js` video-mode
     (`video.currentTime = progress × duration`, אותו אפקט scrub). **הסיבה הייתה
     חמורה יותר מרוחב-פס:** ה-renderer טען בלהיטות (eager, ללא IntersectionObserver
     וללא concurrency-cap) את כל 361 הפריימים בו-זמנית (351 בקשות מקבילות) → (1) הטאב
     **קרס** מ-OOM כי 361 ImageBitmaps של 4K נערמו בזיכרון GPU בדפדפן אמיתי; (2) מבול
     הבקשות חנק את ה-connection-pool וכל שאר תמונות ה-lazy (גלריה/לאונג') נתקעו אפורות.
     **חשוב:** ה-`culinary-1080.mp4` המקורי הוא **HEVC/H.265** ש-Chrome/Firefox/Edge
     **לא יודעים לנגן** (אומת: `MEDIA_ERR_SRC_NOT_SUPPORTED`) — לכן קודד מחדש ל-H.264
     1080p (~5MB). פריימי `assets/frames/culinary/` + `culinary-mobile/` (~220MB) הוחרגו
     מהדיפלוי (skip-set ב-`scripts/deploy-cloudflare.mjs`). תיקון נלווה: ה-`<video>`
     מקבל `videoEl.load()` מפורש ב-`scroll-scene.js` (preload=metadata לבדו לא טוען
     metadata לסרטון offscreen/muted/לא-מנוגן בדסקטופ → duration=0 → scrub לא זז).
