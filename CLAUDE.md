...
| Date       | Change                                                       | Author      |
|------------|--------------------------------------------------------------|-------------|
| 2026-06-09 | **`#kosher` rebuilt as a logo-marquee of 5 real kashrut authorities.** Per-user mandate (Hebrew: "פס של חברות שיש לו אנימציה … חמש חברות כשרות ידועות … עדות המזרח גם אשכנזי וגם דתי לאומי"). The centered SVG-seal block (180×180 `kashrut-seal.svg` + paragraph) was replaced with a horizontal infinite-scroll logo strip on an ivory band. The 5 orgs span the three communities the user requested: **הרבנות הראשית** (state), **צוהר** (Religious-Zionist), **בד"ץ העדה החרדית** (Ashkenazi-Haredi, Jerusalem), **בד"ץ בית יוסף** (Sephardi, Ovadia Yosef tradition), **בד"ץ אגודת ישראל** (Ashkenazi-Haredi). 15-agent SWARM in 3 sequential waves: Wave 0 (5 sourcing agents pulling raw PNG/SVG from each org's site or Hebrew Wikipedia → `GAMOS-DOCS/תמונות לאנימציית האתר/כשרות/`, plus a 6th agent that wrote `mobile/scripts/normalize-kosher-logos.mjs` — sharp pipeline that flattens-on-white → greyscale → linear contrast → threshold(200) → joinChannel to produce 1200-wide black-on-transparent `*-bw.png` masters); Wave 1 (encoder block in `scripts/encode-images.mjs` → `assets/images/kosher/{rabbanut,tzohar,eda-charedit,beit-yosef,agudat-israel}.webp` 600w q92 alpha-preserved, ~108KB total; markup replacement at `index.html:1136-1184`; new `css/sections/kosher.css` ivory-bg shell; new `.marquee--logos` modifier in `css/components/marquee.css` with `filter: brightness(0)` + `opacity: 0.78→1.0 on hover` + `animation-play-state: paused` on container hover; new `mobile/css/kosher.css` registered in `mobile/loader.js` MOBILE_CSS array). Wave 2 (a11y verified: `role="region"` + `aria-label` on shell, Hebrew `alt` per logo, marquee.js auto-aria-hides duplicates; RTL: existing keyframe `translateX(0 → -50%)` already reads right-to-left in RTL per the existing comment in marquee.css:45-48; reduced-motion: existing `@media (prefers-reduced-motion: reduce) { .marquee__track { animation: none !important } }` clause covers the new variant). **Modified:** `index.html` (lines ~1136-1184 markup swap), `css/sections/kosher.css` (full rewrite, 30 lines), `css/components/marquee.css` (append 38-line `.marquee--logos` block), `scripts/encode-images.mjs` (5 new `SINGLE_WEBP` entries), `mobile/loader.js` (1-line addition to `MOBILE_CSS`), `CLAUDE.md` §12 (this row). **New:** `mobile/css/kosher.css` (~30 lines ≤768px overrides), `mobile/scripts/normalize-kosher-logos.mjs` (one-shot sharp pipeline, ~110 lines), `assets/images/kosher/*.webp` (5 files), `GAMOS-DOCS/.../כשרות/{*.png,*.svg,*-bw.png}` (5 raw + 5 normalized + 2 vector source). **Files unchanged:** `js/marquee.js` (consumes the new `<figure>`-children variant verbatim — it duplicates ANY children, applies speed, aria-hides duplicates), `js/main.js` (marquee module already registered), `js/side-dot-nav.js` (`#kosher` dot stays at position 10/12), `assets/img/icons/kashrut-seal.svg` (orphaned, kept on disk for revert). **License/IP note:** the 5 displayed logos are trademarks of their respective issuing bodies. Using them honestly to represent the venue's actual certifications is normal commercial practice for kosher caterers, but the **business owner is responsible** for verifying the venue is in fact certified by each authority shown. The redesign does NOT verify certification status. **Verification (manual, browser-pending):** load `/`, scroll to `#kosher`. Expect ivory full-bleed band, eyebrow "בכשרות מהודרת" + h2 "בהשגחת רבנויות מובחרות" centered above, 5 black logos drifting RTL→LTR (which in RTL reading reads as right→left, the natural Hebrew direction), hover anywhere on band pauses the scroll + each `<figure>` lifts to opacity 1.0 on individual hover. ≤768px: logos shrink, padding tightens, marquee continues. `prefers-reduced-motion: reduce`: marquee freezes mid-state, all 5 logos visible & readable. | main + 6 sourcing/normalization sub-agents |
