  ...
  return (
    <>
      ...
      >
        ...
        <span className="rooms-back__label">{t("חזרה לאתר")}</span>
      </motion.a>
      ...
      >
        <p className="rooms-heading__eyebrow">{t("חדרי אירוח")}</p>
        <p className="rooms-heading__sub">{t("הסוויטות והחדרים של גאמוס")}</p>
      </motion.div>
      ...
      <section aria-live="polite" aria-atomic="true" className="rooms-active">
        ...
        {activeCard?.titleHe ? (
          <p className="rooms-title__type m-0">{t(activeCard.titleHe)}</p>
        ) : null}
      </section>
      ...
      {hintVisible ? (
        ...
        >
          ...
          <span className="rooms-hint__pill">{t("גררו לצדדים כדי לגלות את החדרים")}</span>
